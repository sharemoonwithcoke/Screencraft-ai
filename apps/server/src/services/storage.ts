...
export class StorageService {
  ...
  async uploadChunk(
    ...
    const name = `recordings/${recordingId}/chunks/${String(index).padStart(6, "0")}${ext}`;
  ...
  async downloadOutput(recordingId: string): Promise<Buffer> {
    const name = `recordings/${recordingId}/output.mp4`;
    const [buffer] = await this.storage.bucket(this.bucket).file(name).download();
    return buffer;
  }
