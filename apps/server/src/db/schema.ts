...
export const users = pgTable("users", {
  ...
  passwordHash: varchar("password_hash", { length: 64 }),  // null for OAuth-only users
...
/**
 * scoring_reports — stores the AI quality-score report generated by the Next.js
 * runAnalysis() pipeline (score, issues, speechStats).  Separate from the deep
 * analysis_reports table (which holds visual/audio issues + edit plan for export).
 * One row per recording (unique on recording_id).
 */
export const scoringReports = pgTable("scoring_reports", {
  id: text("id").primaryKey(),
  recordingId: text("recording_id")
    .notNull()
    .unique()
