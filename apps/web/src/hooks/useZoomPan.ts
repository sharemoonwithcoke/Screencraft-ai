...
const HOVER_DELAY_MS = 5000;       // ms the cursor must be fully still before hover-zoom fires
const HOVER_MOVE_THRESHOLD = 0.008; // normalized canvas units — ignore sub-pixel jitter
...
export function useZoomPan(
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const prev = lastHoverPosRef.current;
    const moved =
      Math.abs(nx - prev.nx) > HOVER_MOVE_THRESHOLD ||
      Math.abs(ny - prev.ny) > HOVER_MOVE_THRESHOLD;

    lastHoverPosRef.current = { nx, ny };

    // Only reset the hover timer when the cursor actually moves beyond jitter threshold.
    // This prevents tiny hand tremors from endlessly deferring the zoom.
    if (!moved) return;
